...
export interface CubeStackVisual {
   ...
   * 쌓기나무를 비쳐 보이게 그립니다.
   *
   * 쌓은 모양은 앞의 것이 뒤의 것을 가립니다. 그래서 개수를 물으면
   * 아이가 보이는 것만 세고 뒤에 숨은 것을 빠뜨립니다. 힌트를 열었을
   * 때만 이 값을 켜서, 숨은 것까지 비쳐 보이게 합니다.
   */
  seeThrough?: boolean;
}
...
export interface ArrayVisual {
   ...
   * 앞에서부터 몇 묶음까지 보일지입니다.
   *
   * 자세히 보기에서 한 묶음씩 세어 나갈 때 씁니다. 흐리게만 두면 이미
   * 다 놓여 있는 것을 색만 바꾸는 것이라, 아이 눈에는 처음부터 스무
   * 개가 있는 것으로 보입니다. 없다가 한 묶음씩 나타나야 '더하는 일'이
   * 눈에 들어옵니다.
   */
  shownRows?: number;
  /**
...
export interface AnswerRecord {
  ...
  // 이 문제에서 3단계(조작형) 힌트를 열어 보았는지입니다. 교사가 어느
  // 학생이 어느 문제에서 도움이 더 필요했는지 나중에 살펴볼 수 있습니다.
  hintUsed?: boolean;
}
