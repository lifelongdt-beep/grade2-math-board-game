...
describe('questionFactory', () => {
  ...
  //
  // '여러 가지 방법으로 뺄셈을 해 볼까요'(2-1-u3-l7) 상 수준은 '29 대신
  // 30을 뺀 다음에는?'(답 1을 더합니다)처럼 두 수의 차이를 보여 주면
  // 그것이 곧 답인 문항이 남아 있습니다.
  //
  // 분류하기 앞 세 차시(2-1-u5-l1~l3)는 '단추를 분류하려고 합니다.
  // 분류 기준으로 알맞은 것은?'(답 크기)처럼 나눌 물건을 실제로 보여
  // 주어야 하는데, 크기·색깔·구멍 수가 저마다 다른 물건 더미를 그리는
  // 그림이 아직 없습니다. 그림을 새로 만들면 그때 이 목록에서 지웁니다.
