...
describe('questionFactory', () => {
  ...
  //
  // 하루의 시간(2-2-u4-l7) 하 수준도 마찬가지입니다. '오전은 몇 시간일까요?'
  // (12시간), '밤 12시부터 낮 12시까지를 무엇이라고 할까요?'(오전),
  // '오전과 오후를 나누는 때는?'(낮 12시) — 이 차시의 그림인 하루 띠에
  // 적히는 말이 그대로 답인 문항들입니다. 그리면 답을 그려 주는 셈입니다.
  const noDataToDraw = new Set<string>([
    ...
    '2-2-u4-l7',
    //
    // 덧셈·뺄셈의 '여러 가지 방법으로' 차시와 '관계를 식으로 나타내기'
    // 차시(상 수준)도 같은 까닭입니다. 이 문항들은 수를 내는 것이 아니라
    // 방법을 고릅니다 — '45+20을 쉽게 계산하려고 합니다. 20을 어떻게
    // 가르면 좋을까요?'(답 5와 15), '28을 29로 보고 더한 다음에는?'
    // (답 1을 뺍니다). 가르는 방법을 막대로 그리면 그것이 곧 답이고,
    // 그리지 않으면 그릴 것이 없습니다. 그동안은 '답 5'처럼 답에서 앞
    // 숫자만 떼어 온 막대로 수를 채우고 있었습니다.
    '2-1-u3-l4',
    '2-1-u3-l7',
    '2-1-u3-l9',
    //
    // '세/네 자리 수를 알아볼까요'의 하 수준도 마찬가지입니다.
    // '육백삼십육을 수로 쓰면 얼마일까요?', '100이 6개, 10이 1개,
    // 1이 4개인 수는?' — 수를 그리는 순간 그것이 답입니다. 그동안은
    // 문제에 적힌 수 가운데 가장 큰 것을 그려 수를 채우고 있었는데,
    // 그래서 4257을 만들라는 문제 옆에 1000을 나타내는 표가 놓였습니다.
    '2-1-u1-l4',
    '2-2-u1-l4',
  ]);
