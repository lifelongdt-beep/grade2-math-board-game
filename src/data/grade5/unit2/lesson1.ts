...
export const unit2Lesson1Middle: G5Family[] = [
  ...
  },
  {
    ...
    make: (seed) => {
      const next = rand(seed);
      // 쓴 양을 무엇이라 부르는지도 상황마다 다릅니다 — 물은 마시고
      // 피자는 먹습니다. '쓴 양'으로 뭉뚱그리면 문장이 어색해집니다.
      const 상황 = pick([
        { 글: '물이 1 L 있었는데 그중 얼마를 마셨습니다.', 쓴: '마신 양', 남은: '남은 물' },
        { 글: '색 테이프가 1 m 있었는데 그중 얼마를 썼습니다.', 쓴: '쓴 양', 남은: '남은 색 테이프' },
        { 글: '피자 한 판을 똑같이 나누어 그중 얼마를 먹었습니다.', 쓴: '먹은 양', 남은: '남은 피자' },
      ], seed);
      const d = 4 + next(6);
      const n = 1 + next(d - 2);
      ...
      return {
        prompt: `${상황.글} ${상황.쓴}이 전체의 ${n}/${d}일 때, ${eun(상황.남은)} 전체의 얼마일까요?`,
        answer: text(answer),
        wrongs: [
          `${n}/${d}`,
          text(frac(d - n, d + n)),
...
export const unit2Lesson1Hard: G5Family[] = [
  {
    ...
    make: (seed) => {
      ...
      return {
        ...
        steps: [
          `${gwa(`${n}/${d}`)} ${eul(`${n + 1}/${d}`)} 분모 ${2 * d}${euroText(2 * d)} 통분하면 ${gwa(`${2 * n}/${2 * d}`)} ${2 * n + 2}/${2 * d}입니다.`,
          `그 사이에 ${iJosa(`${2 * n + 1}/${2 * d}`)} 있습니다.`,
          `그러므로 ${text(answer)}입니다.`,
        ],
