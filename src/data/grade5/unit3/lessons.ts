  ...
  make: (seed) => {
    ...
    return {
      ...
      steps: [
        ...
        `각 ${first[(자리 + 2) % 3]}${first[자리]}${first[(자리 + 1) % 3]}의 대응각은 각 ${second[(자리 + 2) % 3]}${second[자리]}${second[(자리 + 1) % 3]}입니다.`,
        `합동인 두 도형에서 대응각의 크기는 같으므로 답은 ${각[자리]}°입니다.`,
      ],
      // 그림이 없으면 아이는 세 각의 합만 셈하고 '대응'은 건너뜁니다.
      // 두 삼각형을 나란히 두고, 첫째에만 아는 두 각을 적습니다.
      // 둘째 삼각형은 돌려 놓습니다 — 돌아가 있어도 대응각을 찾을 수
      // 있어야 한다고 지도서가 적어 두었습니다. 구하는 각에는 아무것도
      // 적지 않습니다.
      visual: {
        ...
        items: [
          {
            shape: '사각형' as const,
            points: 각으로삼각형([각[0], 각[1], 각[2]]),
            vertexLabels: first,
            angleLabels: 각
              .map((one, at) => ({ at, text: `${one}°` }))
              .filter((_, at) => at !== 자리),
          },
          {
            shape: '사각형' as const,
  ...
  삼각형대응각(2),
  {
    ...
    make: (seed) => {
      ...
      return {
        ...
        // 지도서는 대응점을 '이름의 차례'가 아니라 '포개어 겹치는 자리'로
        // 찾게 합니다. 실제로 지도서 본문의 짝은 ㄱ-ㅇ, ㄴ-ㅅ, ㄷ-ㅂ,
        // ㄹ-ㅁ으로 이름의 차례와 거꾸로입니다. '첫째는 첫째끼리'라고
        // 가르치면 교과서 문제에서 그대로 틀립니다.
        hint: '오른쪽 도형을 반 바퀴 돌려 왼쪽 도형에 포개어 보세요. 그 자리에 와서 겹치는 꼭짓점이 대응점입니다.',
        steps: [
          '합동인 두 도형은 포개면 완전히 겹칩니다.',
          `오른쪽 도형을 반 바퀴 돌려 포개면 ${물음}${particleOf(물음, '이')} 놓이는 자리에 ${answer}${particleOf(answer, '이')} 옵니다.`,
...
const 대응각구하기 = (선대칭인가: boolean): G5Family => ({
  ...
  make: (seed) => {
    const next = rand(seed);
    const { first } = 대응이름(4);
    // 90°는 쓰지 않습니다. 90°이면 오답으로 둔 180-각이 정답과 같아져
    // 보기에 정답이 둘 나옵니다. 그리고 도형이 직사각형이 되어 네 각이
    // 모두 같아지므로, 어느 것이 대응각인지 그림에서 가릴 수 없습니다.
    const 뽑은각 = 50 + next(89);
    const 각 = 뽑은각 >= 90 ? 뽑은각 + 1 : 뽑은각;
    const 이름 = 선대칭인가 ? '선대칭도형' : '점대칭도형';
    const 자리 = next(4);
    const 짝 = 선대칭인가 ? 선대칭짝(자리) : 점대칭짝(자리);
    const 묻는각이름 = `${first[(자리 + 3) % 4]}${first[자리]}${first[(자리 + 1) % 4]}`;
    const 짝각이름 = `${first[(짝 + 3) % 4]}${first[짝]}${first[(짝 + 1) % 4]}`;
    return {
      prompt: `${이름}인 사각형 ${first.join('')}에서 각 ${묻는각이름}의 크기가 ${각}°입니다. 그 대응각의 크기는 몇 도일까요?`,
      answer: `${각}°`,
      ...
      // 그림이 없으면 아이는 각 ㄱㄴㄷ이 어느 각인지 볼 수가 없습니다.
      // 꼭짓점은 적어 둔 각도에 맞추어 계산하므로 그림과 글이 어긋나지
      // 않습니다. 대응각 쪽에는 크기를 적지 않습니다 — 적으면 그것이
      // 곧 답입니다.
      visual: {
        kind: 'figure-set' as const,
        label: 이름,
        items: [
          {
            shape: '사각형' as const,
            points: 선대칭인가 ? 선대칭사각형(자리, 각) : 점대칭사각형(자리, 각),
            vertexLabels: first,
            angleLabels: [{ at: 자리, text: `${각}°` }],
...
// 정육각형 ㄱㄴㄷㄹㅁㅂ에서 대응점을 찾습니다.
//
// 예전에는 선대칭이든 점대칭이든 '세 칸 건너뛴 점'이라고 가르쳤습니다.
// 점대칭에서는 맞지만 선대칭에서는 틀립니다. 접어서 겹치는 것은
// 꼭짓점이 도는 차례를 뒤집는데, '세 칸 건너'는 뒤집지 않습니다.
// 그래서 어떤 대칭축으로 접어도 여섯 꼭짓점이 한꺼번에 세 칸씩
// 옮겨 가는 일은 일어나지 않습니다. 정육각형의 대칭축 여섯 개를
// 모두 재어 확인했습니다(symmetryQuad.check.test.ts).
//
// 이 단원 머리말이 "대응점, 대응변, 대응각의 위치를 혼동하지 않도록
// 유의한다"고 적어 둔 바로 그 자리를, 문항이 거꾸로 가르치고
// 있었습니다.
//
//   점대칭 — 대칭의 중심을 사이에 두고 마주 봅니다. ㄱ↔ㄹ, ㄴ↔ㅁ, ㄷ↔ㅂ
//   선대칭 — 세로 대칭축을 사이에 두고 마주 봅니다. ㄴ↔ㅂ, ㄷ↔ㅁ
//            (ㄱ과 ㄹ은 축 위에 있어 저 자신과 겹치므로 묻지 않습니다)
const 대응점이름 = (선대칭인가: boolean): G5Family => ({
  ...
  make: (seed) => {
    ...
    // 선대칭은 축에서 벗어난 네 꼭짓점만 묻습니다.
    const 자리 = 선대칭인가 ? [1, 2, 4, 5][next(4)] : next(6);
    const 짝 = 선대칭인가 ? 6 - 자리 : (자리 + 3) % 6;
    return {
      prompt: `${이름}인 육각형 ${꼭짓점.join('')}입니다. 점 ${꼭짓점[자리]}의 대응점은 무엇일까요?`,
      answer: `점 ${꼭짓점[짝]}`,
      // 선대칭 문항의 첫 오답은 점대칭의 짝입니다. 둘을 헷갈리는 것이
      // 지도서가 적어 둔 바로 그 실수라, 그것을 골라 보게 둡니다.
      wrongs: [...(선대칭인가 ? [(자리 + 3) % 6] : []), 0, 1, 2, 3, 4, 5]
        .filter((one, at, all) => one !== 짝 && one !== 자리 && all.indexOf(one) === at)
        .slice(0, 3)
        .map((one) => `점 ${꼭짓점[one]}`),
      tag: 'congruence',
      strategy: '대응점 찾기',
      hint: 선대칭인가
        ? '대칭축을 따라 접으면 어느 꼭짓점이 어느 꼭짓점 위에 내려앉는지 보세요.'
        : '대칭의 중심을 지나 반대쪽으로 곧게 가면 어느 꼭짓점에 닿는지 보세요.',
      steps: 선대칭인가
        ? [
            `점 ${꼭짓점[자리]}에서 대칭축에 수직으로 선을 긋습니다.`,
            '축에서 같은 거리만큼 반대쪽으로 가면 대응점이 있습니다.',
            `그러므로 대응점은 점 ${꼭짓점[짝]}입니다.`,
          ]
        : [
            '점대칭도형에서 대응점은 대칭의 중심을 사이에 두고 마주 봅니다.',
            `꼭짓점이 여섯 개이므로 점 ${꼭짓점[자리]}의 맞은편은 세 칸 건너뛴 점 ${꼭짓점[짝]}입니다.`,
            `그러므로 대응점은 점 ${꼭짓점[짝]}입니다.`,
          ],
      misconceptionTip: 선대칭인가
        ? '세 칸 건너뛴 점을 고르지 마세요. 접었을 때 겹치는 점이 대응점입니다. 대칭축에서 같은 거리만큼 떨어진 반대쪽을 보세요.'
        : undefined,
      // 그림이 없으면 아이는 ㄱㄴㄷㄹㅁㅂ이 어느 자리에 있는지 알 수
      // 없어, 대응점을 찾는 것이 아니라 규칙을 외우게 됩니다.
      visual: {
        kind: 'figure-set' as const,
        label: 이름,
        items: [
...
const 대응변길이 = (선대칭인가: boolean): G5Family => ({
  ...
  make: (seed) => {
    ...
    const 길이 = 4 + next(20);
    const 이름 = 선대칭인가 ? '선대칭도형' : '점대칭도형';
    // 선대칭 사각형(등변사다리꼴)에서 윗변과 아랫변은 대칭축을 사이에
    // 두고 저 자신과 겹칩니다. 그 변을 물으면 '그 대응변'이 저 자신을
    // 가리켜, 답은 맞지만 묻는 말이 성립하지 않습니다. 축에서 벗어난
    // 두 빗변만 묻습니다.
    const 자리 = 선대칭인가 ? [1, 3][next(2)] : next(4);
    // 선대칭은 오른쪽 빗변과 왼쪽 빗변이 짝입니다(1↔3).
    // 점대칭은 마주 보는 변이 짝입니다.
    const 짝변 = 선대칭인가 ? (자리 === 1 ? 3 : 1) : (자리 + 2) % 4;
    // 변의 길이는 적어 둔 것이 하나뿐이라 어떤 모양으로 그려도 그림과
    // 어긋나지 않습니다. 각은 문제에 적지 않으므로 보기 좋은 값으로 둡니다.
    const 그릴각 = 55 + next(30);
    return {
      ...
      steps: [
        `변 ${first[자리]}${first[(자리 + 1) % 4]}의 대응변은 변 ${first[짝변]}${first[(짝변 + 1) % 4]}입니다.`,
        선대칭인가
          ? '선대칭도형을 대칭축으로 접으면 대응변이 완전히 겹칩니다.'
          : '점대칭도형을 180° 돌리면 대응변이 완전히 겹칩니다.',
        '겹치므로 대응변의 길이는 서로 같습니다.',
      ...
      // 대응변 쪽에는 길이를 적지 않습니다 — 적으면 그것이 곧 답입니다.
      visual: {
        kind: 'figure-set' as const,
        label: 이름,
        items: [
          {
            shape: '사각형' as const,
            points: 선대칭인가 ? 선대칭사각형(0, 그릴각) : 점대칭사각형(0, 그릴각),
            vertexLabels: first,
            edgeLabels: [{ from: 자리, to: (자리 + 1) % 4, text: `${길이} cm` }],
            ...(선대칭인가 ? { axes: ['vertical' as const] } : { center: true }),
          },
        ],
      },
...
const 대응각의합 = (선대칭인가: boolean): G5Family => ({
  ...
  make: (seed) => {
    const next = rand(seed);
    const { first } = 대응이름(4);
    // 90°는 쓰지 않습니다. 도형이 직사각형이 되어 네 각이 모두 같아지면
    // 어느 것이 대응각인지 그림에서 가릴 수 없습니다.
    const 뽑은각 = 40 + next(99);
    const 각 = 뽑은각 >= 90 ? 뽑은각 + 1 : 뽑은각;
    const 이름 = 선대칭인가 ? '선대칭도형' : '점대칭도형';
    const 자리 = next(4);
    const 짝 = 선대칭인가 ? 선대칭짝(자리) : 점대칭짝(자리);
    return {
      ...
      steps: [
        `각 ${first[(자리 + 3) % 4]}${first[자리]}${first[(자리 + 1) % 4]}의 대응각은 각 ${first[(짝 + 3) % 4]}${first[짝]}${first[(짝 + 1) % 4]}입니다.`,
      ...
      // 대응각 쪽에는 크기를 적지 않습니다 — 적으면 더할 것이 남지 않습니다.
      visual: {
        kind: 'figure-set' as const,
        label: 이름,
        items: [
