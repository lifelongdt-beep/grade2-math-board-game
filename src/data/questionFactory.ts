...
//
// 예전에는 '같은 수가 몇 번 있는지 세어 봐요'처럼 무엇을 하라는지는
// 있으나 어디서부터 손대야 할지가 없었습니다. 틀린 아이가 읽고 바로
// 움직일 수 있도록, 첫 동작을 적습니다.
const studentHintGuide: Record<ConceptTag, string> = {
  number: '수직선이나 손가락으로 한 칸씩 세어 보세요. 앞뒤 수가 얼마씩 벌어지는지 먼저 봅니다.',
  placeValue: '수를 천·백·십·일로 갈라 적어 보세요. 자리마다 몇 개인지 눈에 보입니다.',
  addition: '모으는 두 수를 적으세요. 일의 자리끼리 먼저 더하고 10이 넘으면 십의 자리로 하나 올립니다.',
  subtraction: '처음 수와 덜어 낸 수를 적으세요. 일의 자리를 뺄 수 없으면 십의 자리에서 하나 빌려 옵니다.',
  shape: '곧은 선과 뾰족한 곳을 하나씩 짚으며 세어 보세요. 도형은 돌려 놓아도 이름이 바뀌지 않습니다.',
  solid: '1층을 먼저 세고 그 위층을 따로 세어 더하세요. 뒤에 숨은 쌓기나무를 빠뜨리지 마세요.',
  measurement: '물건의 한쪽 끝이 어느 눈금에 있는지 먼저 보고, 두 끝 사이가 몇 칸인지 세어 보세요.',
  classification: '무엇으로 나눌지 먼저 정하세요. 그 기준에 맞는 것만 모으고 센 것에는 표시를 합니다.',
  multiplication: '한 묶음에 몇 개씩인지, 그런 묶음이 몇 개인지 두 수를 먼저 적으세요.',
...
const wordStepQuestion = (
  ...
  if (tag === 'addition' || tag === 'subtraction') {
    ...
    // '몇 개 더 많은지'는 빼서 구하는 상황이라 뺄셈 차시에서만 씁니다.
    // 덧셈 차시에 넣으면 그 차시가 다루는 연산과 어긋납니다.
    if (situation === 2 && !plus) {
      return makeQuestion(
        lesson, difficulty, index,
        `빨간 ${subject(item)} ${a + b}개, 파란 ${subject(item)} ${b}개 있습니다. 빨간 ${topic(item)} 몇 개 더 많은지 구하는 과정입니다. □에 알맞은 수는? ① '몇 개 더 많은지'는 빼서 구합니다. ② 빨간 것은 ${a + b}개, 파란 것은 ${b}개입니다. ③ ${a + b}-${b}=□`,
...
const buildQuestionAt = (lesson: Lesson, difficulty: Difficulty, index: number): Question => {
  ...
  const chosen = guided ??
        (isStepSlot(difficulty, index) && difficulty !== '하'
          ...
          // 응용 문항이 먼저입니다. 응용 문항도 조건을 함께 보는 문항이라
          // 이것을 밀어내면 차시가 받아야 할 응용 문항 몫이 무너집니다.
